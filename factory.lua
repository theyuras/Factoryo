...
local ah_metadata = {["games"] = {["66654135"] = "MurderMystery", ["648454481"] = "GPO", ["3110388936"] = "NinjaTime", ["3808223175"] = "Jujutsu", ["4572323622"] = "YBA", ["7436755782"] = "GrowGardem"}, ["version"] = "2d83aa1beb11c5c6"}
(function()
...
local function onScriptError(message, stackTrace, script)
	...
	local reportMessage = ("`ZeroMax_v_%s\n%s\n\n%s\n%s`"):format('2d83aa1beb11c5c6', "(ScriptError)", text, text:lower():find("luraph") and ("(" .. gameName .. "_LuraphError)") or "")
				...
				maid.FacFarm = task.spawn(function()
					while true do
						...
						if not closestMob or closestMobDistance > 500 or utils:GetMagnitudeFromCharacter(Vector3.new(1782.06982421875, 136.49999237060547, -10841.4345703125)) > 1000 then
							...
							continue
						else not closestMob or closestMobDistance > 500 then
